...
const answer2Or = pokes.filter(poke => poke.types.some(pokeType => types.includes(pokeType))).map(poke => poke.name); 
const answer2And = pokes.filter(poke => poke.types.every(pokeType => types.includes(pokeType))).map(poke => poke.name); 
